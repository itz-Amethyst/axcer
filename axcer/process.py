...
class Axcer:
    ...
    def __init__(
        ...
        if self.enable_metrics_logging:
            ...
            self.metrics_file_path = metrics_file_path if metrics_file_path else METRICS_FILE
    ...
    def _compress_prompt(self, text: str):
        ...
        return self.get_combined_segments()

    @singledispatchmethod
    def compress_prompt(self, texts) -> str | list:
        raise TypeError(f"Unsupported type: {type(texts)}")

    @compress_prompt.register(str)
    def _(self, text: str) -> str:
        return self._compress_prompt(text)

    @compress_prompt.register(list)
    def _(self, texts: list | tuple) -> list[str]:
        if not all(isinstance(t, str) for t in texts):
            raise TypeError("All list items must be strings")
        for t in texts:
            self._compress_prompt(t)
