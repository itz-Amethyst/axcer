...
from experiments.constants.force_tokens import FORCE_TOKENS_FOR_LINGUA2, get_force_token
from experiments.constants.paths import (
    DATASET_PATH,
    DATASET_VOLUME_MOUNT_POINT,
    MERGED_COMPRESSED_LINGUA2_WITH_DATASET_PATH,
...
app = modal.App("axcer_evaluate_llmlingua2", secrets=[hf_secret])
...
# default settings for lingua2
# compress_ratio = 0.5, model_name = microsoft/llmlingua-2-xlm-roberta-large-meetingbank
lingua2_image = lingua2_image.pip_install(
    "huggingface_hub[hf_transfer]==0.34.1",
    extra_index_url="https://download.pytorch.org/whl/cu128",
).env({"HF_HUB_ENABLE_HF_TRANSFER": "1", "TORCH_CUDA_ARCH_LIST": "9.0 9.0a"})


lingua2_image = lingua2_image.uv_pip_install(
    "llmlingua==0.2.2",
    "evaluate==0.4.5",
    "polars==1.31.0",
    "transformers==4.54.0",
...
lingua2_image = lingua2_image.add_local_python_source("experiments", copy=False)
...
class Lingua2Evaluate:
    @modal.enter()
    def setup(
        self,
    ):
        print("Loading llmlingua2 compressor")

        self.llm = PromptCompressor(
    ...
    async def compress_and_save(self, prompts: list[str]):
        ...
        for prompt, dt_name in zip(inputs, dataset_names, strict=False):
            ...
            force_tokens = None
            dt_name = dt_name.removeprefix("processed_")
            if dt_name in FORCE_TOKENS_FOR_LINGUA2:
                force_tokens = get_force_token(dt_name)
            start = time.perf_counter()
            if force_tokens is not None:
                compressed_text = self.llm.compress_prompt(prompt, force_tokens=force_tokens, rate=0.5)
            else:
                compressed_text = self.llm.compress_prompt(prompt, rate=0.5)
        ...
        save_metrics_to_csv(results, dataset_names, PROCESSED_LINGUA2_PATH, model_name=None)
    ...
    def update_dataset_with_compressed_text(self):
        ...
        processed_paths = map_path_to_volume(PROCESSED_LINGUA2_PATH.parent)
        for processed_path in processed_paths.glob("*.csv"):
            ...
            replace_input_column_values(processed_path, dataset_path, MERGED_COMPRESSED_LINGUA2_WITH_DATASET_PATH, base_name)


@app.local_entrypoint()
async def main():
    datasets = load_datasets(DATASET_PATH, question_col="input_field", answer_col="answer_field")
    lingua2_e = Lingua2Evaluate()
    ...
    async def process_dataset_selective(dataset_name, df, question_col):
        ...
        async for _batch in lingua2_e.compress_and_save.map.aio(prompts):
            pass
